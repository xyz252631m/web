...
//数值映射
function visualMap(list,min,max){
    ...
    var _min = Math.min.apply([],list.map(function(d){return d.value}));
    var _max = Math.max.apply([],list.map(function(d){return d.value}));

    var src_range = [_min,_max];
    var range = [min,max];
    list.forEach(function(d){
        d.num = d.value;
        d.value = convertToRange(d.num,src_range,range);
    })
    return list;
}
//表操作（创建、替换、删除。。。）`
