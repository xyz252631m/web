    ...
    tickProto.render = function (index, old, opacity) {
        ...
        while (group.parent) {
            ...
            if (!isNaN(attrs.x) && !isNaN(attrs.y)) {
                ...
                if (horiz && tickPos === group.startAt && group.startAt + group.childCount !== axis.categories.length) {
                    if (horiz) {
                        addGridPart(grid, [maxPos.x - reverseCrisp, start, maxPos.x - reverseCrisp, size + lvlSize], tickWidth);
                    } else if (!horiz && axis.top <= maxPos.y) {
                        addGridPart(grid, [start, maxPos.y + reverseCrisp, size + lvlSize, maxPos.y + reverseCrisp], tickWidth);
                    }
                }
            }
