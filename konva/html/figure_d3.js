function Figure(SVG, option) {
    ...
    $(window).on("mousedown.relation", function (e) {
        ...
        if (!self.$box[0].contains(e.target)) {
            return;
        }
        isDown = true;
        isMove = false;
    ...
    $(window).on("mousemove.relation", function (e) {
        ...
        .on("mouseup", function () {
            ...
            if (self.dragNode.isDown) {
                ...
                //     }
                // }
                self.dragNode.item.fx = null;        //固定坐标清空
                self.dragNode.item.fy = null;
                if (self.opt.moveUpFn) {
                    self.opt.moveUpFn.call(self);
                }
            }
            if (isDown) {
                if (!isMove) {
...
$.extend(Figure.prototype, {
    ...
    renderNode: function (item) {
        var self = this;
        var g = self.up_node.group().transform({x: item.x - 33.5, y: item.y - 33.5}).addClass("node-company");
        item.g = g;

        var icon_g = g.group();
        icon_g.use(this.draw.defs().select("#citySpace").first().size(30, 30))
        ...
        var text = g.text(item.name).font({size: 12}).x(10).y(55);


        //text.x(33).y(30).attr("text-anchor", "middle");
    ...
    stepUpdate: function (nodes, links) {
        ...
        links.forEach(d => {
            // if( drag.isMove){
            //     if(drag.item===d.source){
            //         return;
            //     }
            //     if(drag.item===d.target){
            //         return;
            //     }
            // }
            if (d.line) {
    ...
    renderLink: function (item) {
        var self = this;
        var g = self.up_line.group().addClass("node-link-space");
        item.g = g;
        var x = item.source.x;
        var y = item.source.y;
        ...
        var text = g.text("").x(x + (x1 - x) / 2).y(y + (y1 - y) / 2 - 6).rotate(deg);
        item.textNode = text;
