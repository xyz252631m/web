angular.module('starter.services', [])
    ...
    .factory("catalog", function ($cordovaFile, $ionicPopup, $q, check, tool) {

        //cordova.file.dataDirectory = cordova.file.cacheDirectory;
        function writeFile(fileName, jsonStr, flag) {
            $cordovaFile.writeFile(cordova.file.dataDirectory, fileName, jsonStr, flag)
                .then(function (success) {
                }, function (error) {
                    $ionicPopup.alert({template: "缓存目录(json)失败！"});
                });
        }

        return {
            save: function (typeName, bookId, jsonStr) {
                if (check.isWin32) {
                    var deferred = $q.defer();
                    try {
                        localStorage["temp"] = jsonStr;
                        deferred.resolve();
                    } catch (e) {
                        deferred.reject(e);
                    }
                    ...
                    //cordova.file.dataDirectory = "filesystem:"+location.origin+"/persistent";
                    var name = typeName + "/" + bookId;
                    $cordovaFile.checkDir(cordova.file.dataDirectory, name)
                        .then(function (success) {
                            writeFile(name + "/catalog.json", jsonStr, true);
                        }, function (error) {
                            $cordovaFile.createDir(cordova.file.dataDirectory, name, false)
                                .then(function (success) {
                                    writeFile(name + "/catalog.json", jsonStr, true);
                                }, function (error) {
                                    $ionicPopup.alert({template: "创建缓存目录(二级)失败！"});
                                });
                        });
                }
            },
            get: function (typeName, bookId) {
                if (check.isWin32) {
                    var deferred = $q.defer();
                    try {
                        var res = localStorage["temp"];
                        if (res) {
                ...
                var name = typeName + "/" + bookId;
                return $cordovaFile.readAsText(cordova.file.dataDirectory, name + "/catalog.json");
            }
        }
    })
    //类型列表
    .factory("bookTypeList", function (lc, lcSort, ajax, $q, tool, catalog) {
        function op() {
        }
        ...
        op.prototype = {
            ...
            getIndexTabList: function (temHtml) {
                ...
                $.each(temList, function (idx, item) {
                    ...
                    //one
                    var $ul = $box.finds("ul"),
                        block = $ul.finds(".block_img"),
                        block_txt = $ul.finds(".block_txt");
                    var one = {
    ...
    .factory("books", function (bookTypeList, lc) {
        var _nameList;
        return {
            getType: function () {
    ...
    //localStorage 方法集合
    .factory("lc", function ($q) {
        return {
            //localStorage name 集合
            name: {
                type: "type",
                bookshelf: "bookshelf",
                startNum: "startNum",
    ...
    .factory("lcSort", function (lc) {
        return {
            get: function (typeName) {
                try {
                    var str = lc.get(lc.name.sort + typeName);
                    if (str) {
            ...
            set: function (typeName, list, isRefresh) {
                try {
                    var str = lc.get(lc.name.sort + typeName);
                    if (!str || isRefresh) {
                        lc.set(lc.name.sort + typeName, JSON.stringify(list));
                    }
                    return 1;
                } catch (e) {
                    return 0;
            ...
            isExistSort: function (typeName) {
                return lc.get(lc.name.sort + typeName);
            }
        }
    })
    .factory("bookshelf", function () {
    });
