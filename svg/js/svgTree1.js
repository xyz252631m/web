mi.define("ready2", function () {
    return {
        init: function (SVG, CiteTree) {
            ...
            var nameList = ["高大山", "谢大海", "马宏宇", "林莽", "黄强辉", "章汉夫", "范长江", "林君雄", "谭平山", "朱希亮", "李四光", "甘铁生", "张伍绍祖", "马继祖", "程孝先", "宗敬先",
                ...
                "张成基", "张国柱", "张志远", "张广才", "吕德榜", "吕文达", "吴家栋", "吴国梁", "吴立功", "李大江", "张石山", "王海"];
            let tagList = ['在业', '高新技术企业', '天使轮', '建筑工程企业', 'A股'];
            let rangeBox = {
                //滑条总高度
...
mi.define("ready", async () => {
    let SVG = await mi.preload("svg", "../svg.min.js");
    console.log("SVG", SVG);
    let citeTree = mi.preload("citeTree", "../js/citeTree.js");
    let { CiteTree } = await citeTree;
    let ready2 = await mi.use("ready2");
    $(function () {
        ready2.init(SVG, CiteTree);
    });
});
